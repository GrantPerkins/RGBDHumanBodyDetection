...
class Dataset:
    ...
    def __init__(self, a_=6, s_=10, e_=1, crop=True, length=40, width=40):
        """
        ...
        self._generate_videos(crop, a_, s_, e_)
        self.i = 0
        self.images = 0

    def _generate_videos(self, crop, a_, s_, e_):
        """
        ...
        for a in range(a_):  # out of 2
            if True:  # out of 10
                for e in range(e_):  # out of 1 (value = 2)
                    ...
                    print(round(((time() - start) / ((percent * 60) + 1e-10)) - ((time() - start) / 60)),
                          "minutes remaining")
