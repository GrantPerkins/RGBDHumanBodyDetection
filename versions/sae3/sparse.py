...
class SparseAutoEncoder:
    ...
    def train(self, data, train_steps=1000):
        # 1365.569870710373
        """
        Trains autoencoder
        ...
        save_path = self.saver.save(self.sess, "/var/sae_old1.cpkt")
    ...
    def save_weight_picture(self, plots_len=4, file_name="trained_weights_2.png"):
        ...
        for i, axis in enumerate(axes.flat):
            axis.imshow(images[i + 0, :].reshape(self.length, self.width), cmap=plt.get_cmap("binary"))
            axis.set_axis_off()
        plt.savefig(file_name)
        print("Picture of weights saved to", file_name)
        plt.imshow(images[0, :].reshape(self.length, self.width), cmap=plt.get_cmap("binary"))
...
def main():
    ...
    """
    length, width = 20, 20
    # data = Dataset(6, 10, 1, True, length, width)
